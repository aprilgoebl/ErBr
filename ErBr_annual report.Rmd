...
ggplot(precip[precip$Prev12>1994&precip$Prev12<=currentyr,], aes(Prev12, ppt))+
  ...
  annotate("text", x=1998, y=425, label=paste(round(mean(precip$ppt),0),"mm", sep=""))+
  theme_bw()+
...
ggplot(mintemp, aes(Prev12, tmin))+
  ...
  annotate("text", x=1995, y=mean(mintemp$tmin)+0.06, label= "2.3") +
  theme_bw()+
  geom_rect(aes(xmin=-Inf,xmax=2003.5,
                ymin=-Inf, ymax=Inf),
            fill= "grey", alpha = 0.03)+
...
ggplot(maxtemp, aes(Prev12, tmax))+
  ...
  annotate("text", x=1994.7, y=mean(maxtemp$tmax)+0.06, label= "17.95") +
  theme_bw()+
  geom_rect(aes(xmin=-Inf,xmax=2003.5,
                ymin=-Inf, ymax=Inf),
            fill= "grey", alpha = 0.03)+
...
Fixed-effects model
take x predictor (tmin) and split into two predictors, tminb  which will contain only between-cluster variation in x, and xw which will contain only within-cluster variation in x.
```{r}
#Average per site (which is the same as average over the years)
erbr.dat <- within(erbr.all, tminb <- tapply(tmin, Site, mean)[paste(Site)])
#Difference each year from the average
erbr.dat <- within(erbr.dat, tminw <- tmin - tminb)

erbr.mod <- lmer(logSumOfRosettes ~ tminb + tminw + (1|Site), data=erbr.dat)
erbr.mod
```
...
rm(erbr.dat)
# average per site y
erbr.dat <- within(erbr.all, 
                   logSumOfRosettesavg <- tapply(logSumOfRosettes, Site, mean)[paste(Site)])
erbr.dat <- unique(erbr.dat[,c("Year","Site","logSumOfRosettesavg","tmin")])

lmer(logSumOfRosettesavg ~ tmin + (1|Site), data = erbr.dat)
